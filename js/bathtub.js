...
// 		n_o			-	QUANTxQUANT array. initial value for column heights.
//		u_o, v_o	-	same. IV for velocities. must conform to reflective BCs above.
function SWE(QUANT, LENGTH, n_o, u_o, v_o, g) {
	...
	var drop = function(x, y, i, j) {
		return 100 * Math.exp(-0.08 * ((x - i) * (x - i) + (y - j) * (y - j)));
	};
...
function CanvasBathtub(ctnr) {
	...
	this.canvas.onclick = function(e) {
		var x = Math.floor(Math.floor(e.offsetX / system.dd));
		var y = Math.floor(Math.floor(e.offsetY / system.dd));
